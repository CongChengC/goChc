...
func init() {
	fmt.Println("----开始启动Redis连接池----")
	//当服务器启动时，我们就去初始化我们的redis的连接池
	service.InitPool("localhost:6379", 16, 0, 300*time.Second)
	initUserDao()
	fmt.Println("----Redis连接池启动完成....")
}
...
func initUserDao() {
	...
	model.MyUserDao = model.NewUserDao(service.RedisPool)
}
